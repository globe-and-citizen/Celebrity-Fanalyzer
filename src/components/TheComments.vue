    ...
    <div v-for="comment of comments" class="q-mb-md" :key="comment.id">
      <div v-if="comment.parentId === undefined">
          ...
          >
            <q-list>
              ...
              <q-item
                ...
                clickable
                v-close-popup
                @click="deleteComment(1, comment.id)"
              >
                <q-item-section>
        ...
        >
          ...
          >
            <q-btn
              data-test="submit-edited-comment"
...
const dislikeIconClass = computed(() => {
  return (comment) =>
    comment.dislikes?.some((dislike) => dislike === userId.value) || false;
});
...
async function deleteComment(commentParentId, commentId) {
  ...
  childComments.value = props.comments.filter(
    (comment) => commentParentId === comment.parentId
  );
}
